...
function loadGameData(gameData, isInit) {
    N_SIZE = gameData.game_size;

    score = {
        X: gameData.players.tic.length,
        O: gameData.players.toc.length,
    };
    moves = score.X + score.O;
    turn = score.X == score.O ? 'X' : 'O';
...
async function init() {
    getBoardGame()
        .then((gameData) => {
            if (!gameData) {
                try {
                    gameData = await createBoardGame();
                } catch (e) {
                    console.error(e);
                }
